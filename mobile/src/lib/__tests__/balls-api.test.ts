...
describe("BallsApi", () => {
  it("adds bearer and optional session-key headers to authenticated requests", async () => {
    ...
    expect(calls[0].init?.headers).toEqual({
      ...
      "X-Balls-Session-Key": "session-scope",
    });
  });
  ...
  it("creates a run from input and optional session id", async () => {
    ...
    await expect(api.startRun({ input: "Hello Balls", sessionId: "session-1" })).resolves.toEqual({
      runId: "run-1",
      status: "started",
    });
    ...
    const api = createBallsApi({
      ...
      fetchImpl: async (input, init) => {
        ...
        if (String(input).endsWith("/api/sessions") && init?.method === "POST") {
          return jsonResponse({ object: "balls.session", session: { id: "session-1" } }, 201);
        }
  ...
  it("normalizes an HTTP error without exposing the bearer token", async () => {
    ...
    expect(error).toBeInstanceOf(BallsApiError);
