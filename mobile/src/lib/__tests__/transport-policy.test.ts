...
import { DEFAULT_HERMES_API_URL, resolveBallsApiUrl, resolveAttachmentAdapterUrl, DEFAULT_ATTACHMENT_ADAPTER_URL } from "../transport-policy";

describe("resolveBallsApiUrl", () => {
  it("accepts the local Termux endpoint", () => {
    expect(resolveBallsApiUrl("http://127.0.0.1:8642")).toBe("http://127.0.0.1:8642");
    expect(resolveBallsApiUrl("http://localhost:8642")).toBe("http://localhost:8642");
  });

  it("accepts the Android emulator host bridge", () => {
    expect(resolveBallsApiUrl("http://10.0.2.2:8642")).toBe("http://10.0.2.2:8642");
  });

  it("falls back to loopback for remote, IPv6, or malformed endpoints", () => {
    expect(resolveBallsApiUrl("http://example.com:8642")).toBe(DEFAULT_HERMES_API_URL);
    expect(resolveBallsApiUrl("https://api.example.com")).toBe(DEFAULT_HERMES_API_URL);
    expect(resolveBallsApiUrl("http://[::1]:8642")).toBe(DEFAULT_HERMES_API_URL);
    expect(resolveBallsApiUrl("not a URL")).toBe(DEFAULT_HERMES_API_URL);
  });
});
