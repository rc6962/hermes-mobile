...
export interface FakeBallsServerOptions {
  apiKey?: string;
}

export interface FakeBallsServer {
  readonly url: string;
  start(): Promise<void>;
  stop(): Promise<void>;
}
...
export function createFakeBallsServer(options: FakeBallsServerOptions = {}): FakeBallsServer {
  ...
  const handle = async (request: IncomingMessage, response: ServerResponse): Promise<void> => {
    ...
    if (request.method === "GET" && pathname === "/v1/capabilities") {
      writeJson(response, {
        object: "balls.api_server.capabilities",
        platform: "balls-agent-fixture",
    ...
    if (pathname === "/api/sessions" && request.method === "POST") {
      ...
      writeJson(response, { object: "balls.session", session }, 201);
      return;
    }
    ...
    if (request.method === "POST" && approvalMatch) {
      ...
      run.status = "running";
      writeJson(response, {
        object: "balls.run.approval_response",
        run_id: runId,
        choice,
        resolved: 1,
      });
